...
happyExpList = Happy_Data_Array.listArray (0,37) ([13312,6656,48,96,0,416,0,49152,0,26,6157,16385,40963,1,0,0,384,192,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_bottomUp","E","T","F","'+'","'-'","'*'","'/'","'('","')'","int","id","%eof"]
        bit_start = st * 15
        bit_end = (st + 1) * 15
...
happyReduction_8 (HappyTerminal (LexInt happy_var_1))
	 =  HappyAbsSyn6
		 (Int happy_var_1
	)
...
happyReduction_9 (HappyTerminal (LexId happy_var_1))
	 =  HappyAbsSyn6
		 (Id happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	LexLParen -> cont 11;
	LexRParen -> cont 12;
	LexInt happy_dollar_dollar -> cont 13;
	LexId happy_dollar_dollar -> cont 14;
	_ -> happyError' ((tk:tks), [])
	}
...
happyError' :: () => ([(Lexeme)], [String]) -> HappyIdentity a
happyError' = HappyIdentity . (\(tokens, _) -> parseError tokens)
bottomUp tks = happyRunIdentity happySomeParser where
...
{-# LINE 17 "<built-in>" #-}
{-# LINE 1 "/var/folders/5l/j8h3l8hj4110mydjv_xyrbgh0000gn/T/ghc32365_0/ghc_2.h" #-}
